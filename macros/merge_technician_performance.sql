-- {% macro merge_technician_performance() %}

--     -- Remove duplicates from stage table
--     DELETE FROM NC_TRAILERS.BLACKPURL_PRODUCTION.TECHNICIAN_PERFORMANCE_STAGE main
--     WHERE EXISTS (
--     SELECT 1
...
--     -- Remove data from prod table. Merge statements must be unique
--     DELETE FROM NC_TRAILERS.BLACKPURL_PRODUCTION.TECHNICIAN_PERFORMANCE main
--     WHERE EXISTS (
--     SELECT 1
--     FROM NC_TRAILERS.BLACKPURL_PRODUCTION.TECHNICIAN_PERFORMANCE_STAGE sub
--     WHERE COALESCE(TRY_TO_DATE(main.DATE, 'MM/DD/YYYY'), TRY_TO_DATE(main.DATE, 'M/D/YYYY'), TRY_TO_DATE(main.DATE, 'MM-DD-YYYY'), TRY_TO_DATE(main.DATE, 'M-D-YYYY')) = 
...
--     MERGE INTO NC_TRAILERS.BLACKPURL_PRODUCTION.TECHNICIAN_PERFORMANCE prod
--     USING NC_TRAILERS.BLACKPURL_PRODUCTION.TECHNICIAN_PERFORMANCE_STAGE stage
...
--     TRUNCATE TABLE NC_TRAILERS.BLACKPURL_PRODUCTION.TECHNICIAN_PERFORMANCE_STAGE;
